...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        */
        ...
        if(keys.contains(key)) throw new BPlusTreeException("You can't add duplicate key!");

        int idx = InnerNode.numLessThan(key,keys);
        keys.add(idx,key);
        rids.add(idx,rid);

        if(keys.size() > metadata.getOrder() * 2){//需要分裂

            List<DataBox> rkeys = new ArrayList<>();
            List<RecordId> rrids = new ArrayList<>();
            int order = metadata.getOrder();

            rkeys = keys.subList(order, keys.size());
            rrids = rids.subList(order, rids.size());
            ...
            DataBox splitKey = rkeys.get(0);
            Long rpageNum = rNode.getPage().getPageNum();
            rightSibling = Optional.of(rpageNum);
    ...
    @Override
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data/*java的Iterator是按引用传递*/ ,
            float fillFactor) {
        // TODO(proj2): implement
        
        int maxLoadOrder = (int)Math.ceil(metadata.getOrder()*2*fillFactor);
        Pair<DataBox, RecordId> info;
        DataBox key;
        RecordId rid;
        while(data.hasNext() && keys.size() < maxLoadOrder){
            info = data.next();
            key = info.getFirst();
            rid = info.getSecond();
            keys.add(key);
        ...
        if(keys.size() == maxLoadOrder && data.hasNext()){
            List<DataBox> rkeys = new ArrayList<>();
            List<RecordId> rrids = new ArrayList<>();
            info = data.next();
            key = info.getFirst();
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        Long Siblingtmp = buf.getLong();
        if(Siblingtmp == -1) Siblingtmp = null;
        Optional<Long> rightSibling = Optional.ofNullable(Siblingtmp);
