...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        while(records.hasNext()) list.add(records.next());
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        List<Iterator<Record>> runsIterator = new ArrayList<>();//保存每个run的iterator

        for(int i = 0; i < runs.size(); i++){//初始化
            runsIterator.add(runs.get(i).iterator());
            pq.add(new Pair<>(runsIterator.get(i).next(),i));
        }

        while(!pq.isEmpty()){
            Record record = pq.element().getFirst();
            int indexOfRun = pq.element().getSecond();
            pq.remove();
            mergedRun.add(record);
            if(runsIterator.get(indexOfRun).hasNext()){
                pq.add(new Pair<>(runsIterator.get(indexOfRun).next(), indexOfRun));
            }
        }
        // TODO(proj3_part1): implement
        return mergedRun;
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        while(index < N){
            ...
            int length = N-index+1 >= numBuffers - 1 ? numBuffers - 1 : N-index+1; 
            for(int i = 1; i <= length; i++){
                tmpRuns.add(runs.get(index));
                index++;
            }
            resRuns.add(mergeSortedRuns(tmpRuns));
        }
        return resRuns;
    }
    ...
    public Run sort() {
        ...
        Iterator<Record> buffer = getBlockIterator(sourceIterator, getSchema(), numBuffers);
        while(buffer.hasNext()){
            runs.add(sortRun(buffer));
            buffer = getBlockIterator(sourceIterator, getSchema(), numBuffers - 1);
        }
        while(runs.size() > 1){
            runs = mergePass(runs);
        }
