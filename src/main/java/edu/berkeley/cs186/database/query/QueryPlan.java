...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int except = -1;
        for(int IndexColumn: EligibleIndexColumns){
            SelectPredicate currPredicate = selectPredicates.get(IndexColumn);
            QueryOperator currIndexScanOperator = new IndexScanOperator(transaction, table, 
                        currPredicate.column, currPredicate.operator, currPredicate.value);
            int currIOCost = currIndexScanOperator.estimateIOCost();
            if(currIOCost <= minIOCost){
                minOp = currIndexScanOperator;
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for(Set<String> prevTables: prevMap.keySet()){
            ...
            for(JoinPredicate predicate: joinPredicates){
                String lTable = predicate.leftTable;
                String rTable = predicate.rightTable;
                String lColumn = predicate.leftColumn;
                String rColumn = predicate.rightColumn;

                QueryOperator minCostJoinOperator = null;
                Set<String> resultkey = new HashSet<>();

                if(prevTables.contains(lTable) && !prevTables.contains(rTable)){
                    QueryOperator leftOp = prevMap.get(prevTables);
                    QueryOperator rightOp = pass1Map.get(new HashSet<String>(){{add(rTable);}});
                    minCostJoinOperator = minCostJoinType(leftOp, rightOp, lColumn, rColumn);
                    
                    resultkey.addAll(prevTables);
                    resultkey.add(rTable);
                    result.put(resultkey, minCostJoinOperator);
                }
                else if(!prevTables.contains(lTable) && prevTables.contains(rTable)){
                    QueryOperator leftOp = pass1Map.get(new HashSet<String>(){{add(lTable);}});
                    QueryOperator rightOp = prevMap.get(prevTables);
                    minCostJoinOperator = minCostJoinType(leftOp, rightOp, lColumn, rColumn);

                    resultkey.add(lTable);
                    resultkey.addAll(prevTables);
                    result.put(resultkey, minCostJoinOperator);
                }
                else continue;
    ...
    public Iterator<Record> execute() {
        ...
        for(String table: tableNames){
            pass1Map.put(new HashSet<String>(){{add(table);}}, minCostSingleAccess(table));
        }
        prevMap.putAll(pass1Map);
