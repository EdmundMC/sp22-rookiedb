...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Long rpageNum = splitInfo.get().getSecond();

        List<DataBox> newKeys = new ArrayList<>();
        List<Long> newChildren = new ArrayList<>();
        newKeys.add(splitKey);
        newChildren.add(metadata.getRootPageNum());
        newChildren.add(rpageNum);
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        // the tree's root if the old root splits.
        while(data.hasNext()){
            Optional<Pair<DataBox,Long>> splitInfo = root.bulkLoad(data,fillFactor);//LeafNode会自动.next()
            if(splitInfo.isEmpty()) continue;
            DataBox splitKey = splitInfo.get().getFirst();
            Long rpageNum = splitInfo.get().getSecond();
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator(LeafNode newNode){
            node = newNode;
            iter = newNode.scanAll();
        }

        public BPlusTreeIterator(LeafNode newNode, DataBox key){
            node = newNode;
            iter = newNode.scanGreaterEqual(key);//注意，可能不存在key元素
        }

        @Override
        public boolean hasNext() {
            // TODO(proj2): implement
            if(!iter.hasNext() && node.getRightSibling().isEmpty()) return false;
            return true;
        }
