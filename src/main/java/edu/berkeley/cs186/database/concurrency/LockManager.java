...
public class LockManager {
    ...
    private class ResourceEntry {
        ...
        public void grantOrUpdateLock(Lock lock) {
            ...
            List<Lock> transactionLock = transactionLocks.get(transNum);
            for(Lock curr: locks){
                if(curr.transactionNum == transNum){
                    //更新resource上的
                    curr.lockType = lock.lockType;
                    //更新transaction上的
                    for(Lock tmp: transactionLock){
                        if(tmp.name == lock.name){
                            transactionLock.set(transactionLock.indexOf(tmp), new Lock(tmp.name, lock.lockType, tmp.transactionNum));
                            break;
                        }
                    }
                    return;
    ...
    public void acquireAndRelease(TransactionContext transaction, ResourceName name,
        ...
        synchronized (this) {
            ...
            Lock currLock = new Lock(name, lockType, transNum);                      
            ...
            if(entry.checkCompatible(lockType, transNum)){
                ...
                for(ResourceName releaseName: releaseNames){
                    ...
                    releasedLocks.add(new Lock(name, releasedEntry.getTransactionLockType(transNum), transNum));
                }
                LockRequest request = new LockRequest(transaction, currLock, releasedLocks);
    ...
    public void acquire(TransactionContext transaction, ResourceName name,
        ...
        synchronized (this) {
            ...
            if(entry.checkCompatible(lockType, transNum) && entry.waitingQueue.isEmpty()){
                entry.grantOrUpdateLock(addLock);
            }
            else{
                shouldBlock = true;
                LockRequest request = new LockRequest(transaction, addLock);
    ...
    public void promote(TransactionContext transaction, ResourceName name,
        ...
        synchronized (this) {
            long transNum = transaction.getTransNum();
            ResourceEntry entry = getResourceEntry(name);
            Lock currLock = new Lock(name, newLockType, transNum);
            LockType oldType = entry.getTransactionLockType(transNum);

            if(oldType == newLockType){
                throw new DuplicateLockRequestException("");
            }
            ...
            if(entry.checkCompatible(newLockType, transNum)){
                entry.grantOrUpdateLock(new Lock(name, newLockType, transNum));
            }
            else{
                shouldBlock = true;
                LockRequest request = new LockRequest(transaction, currLock);
                entry.addToQueue(request, true);
                transaction.prepareBlock();
            }
        }
